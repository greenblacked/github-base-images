#!/usr/bin/env bash
# Smoke tests for the ci-python314 image. Run against a built image before it is pushed.
#   ./ci-python314/test.sh ci-python314:test
#
...
check "python is 3.14"             '[ "$(python -c "import sys; print(\"%d.%d\" % sys.version_info[:2])")" = 3.14 ]'
