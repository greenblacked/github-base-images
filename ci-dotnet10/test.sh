#!/usr/bin/env bash
# Smoke tests for the ci-dotnet10 image. Run against a built image before it is pushed.
#   ./ci-dotnet10/test.sh ci-dotnet10:test
#
...
check "dotnet sdk is 10"           'dotnet --version | grep -q "^10\."'
check "dotnet lists an sdk"        'dotnet --list-sdks | grep -q "^10\."'
