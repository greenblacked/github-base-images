#!/usr/bin/env bash
# Smoke tests for the ci-node24 image. Run against a built image before it is pushed.
#   ./ci-node24/test.sh ci-node24:test
#
...
check "node is v24"                '[ "$(node -p "process.versions.node.split(\".\")[0]")" = 24 ]'
...
check "TLS verification works"     'curl -sSf --max-time 15 https://registry.npmjs.org/ -o /dev/null'
