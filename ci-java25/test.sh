#!/usr/bin/env bash
# Smoke tests for the ci-java25 image. Run against a built image before it is pushed.
#   ./ci-java25/test.sh ci-java25:test
#
...
check "java is 25"                 'java --version | head -1 | grep -qE " 25[. ]"'
...
check "TLS verification works"     'curl -sSf --max-time 15 https://repo1.maven.org/maven2/ -o /dev/null'
